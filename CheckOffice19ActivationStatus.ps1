# Intro
Clear-Host;
Write-Host "Checking Office 2019 activation status on listed PCs" -ForegroundColor Blue -BackgroundColor White;
Write-Host "NB: Failure may be due to Office not being present on the target device.`n";
...
function CheckOfc19Activation {
# - For Loop
foreach ($PC in $Global:list) {
Invoke-Command -ComputerName $PC -Credential $Global:cred -ScriptBlock {$Status = (Get-CimInstance -ClassName SoftwareLicensingProduct -Filter "Name like 'Office 19%'" | where PartialProductKey).licensestatus;
If ($Status -ne 1) {Write-Host "Office is NOT activated on $env:computername" -ForegroundColor DarkRed -BackgroundColor White} ElseIf ($Status -eq 1) {Write-Host "Office is activated on $env:computername" -ForegroundColor Green}
}
};
}
